...
class PromptTemplates:
    ...
    CONTEXT_SELECTION = PromptTemplate(
        (
            "És um farmacêutico especializado responsável por identificar as informações mais relevantes "
            "sobre medicamentos para responder a consultas.\n\n"
            "CONSULTA DO USUÁRIO: {query}\n\n"
            "Abaixo estão vários fragmentos de informações disponíveis. Avalie cada fragmento "
            "e identifique os {top_n} mais relevantes para responder à consulta com precisão.\n\n"
            "CRITÉRIOS DE AVALIAÇÃO:\n"
            "1. Relevância direta para os medicamentos ou substâncias mencionados na consulta\n"
            "2. Informações sobre indicações terapêuticas, dosagem, efeitos secundários, contraindicações, etc.\n"
            "3. Informações regulatórias e técnicas relevantes\n"
            "4. Atualidade e precisão técnica das informações\n"
            "5. Informações específicas vs. informações genéricas (prefira informações específicas)\n\n"
            "{fragments}\n\n"
            "Responda APENAS com uma lista de números separados por vírgulas, indicando os índices dos "
            "{top_n} fragmentos mais relevantes em ordem decrescente de relevância.\n"
            "Exemplo: 7,12,3,9,2\n\n"
            "NÃO ADICIONE NENHUM TEXTO EXPLICATIVO - APENAS OS ÍNDICES."
        )
    )
...
class MedicationIndexRetriever:
    ...
    def create_query_engine(self, filters_qdrant):
        ...
        context_selector = ContextSelectorPostProcessor(
            llm=self.llm,
            prompt_template=PromptTemplates.CONTEXT_SELECTION,
            top_n=15  # Limitar aos 15 melhores contextos
        )
